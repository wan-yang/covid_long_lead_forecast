...
t1 = Sys.time()
{
# tmp = res.eval %>% filter(!fcast.deflat %in% c(1,.9))
# acc.1.v.095 = res.eval %>% filter(fcast.deflat %in% c(1,0.95)) %>%  # 0.95 somehow not selected
acc.1.v.095 = res.eval %>% filter(fcast.deflat != .9)  %>%
...
acc.1.v.095$week.t = acc.1.v.095$fcast.start.week + as.numeric(gsub('wk ahead', '', acc.1.v.095$target)) * 7
wave.t = acc.1.v.095[, list(wave = fn_getWave(week.t = week.t, loc.t = loc, vdates.dom = vdates.dom, variant = variant)), by = c('loc', 'variant', 'week.t')]
acc.1.v.095 = acc.1.v.095 %>% left_join(x = ., y = wave.t, by = c('loc', 'variant', 'week.t')) %>% data.table()

acc.1.v.09 = res.eval %>% filter(fcast.deflat %in% c(1,.9)) %>%
  melt(., id.vars =  c('loc', 'variant', 'fcast.type', 'fcast.deflat', 'fcast.start.week','measure', 'metric', 'target', 'scenario', 'seasonality')) %>%
  dcast(., loc + variant + fcast.type + seasonality + measure + metric + fcast.start.week + target + scenario + variable ~ fcast.deflat) %>% 
  filter(variable == 'mean') %>%
  mutate(target2 = factor(target, levels = c(paste0(1:26, 'wk ahead'), 'peak intensity', 'peak week', 'total'),
...
acc.1.v.09$week.t = acc.1.v.09$fcast.start.week + as.numeric(gsub('wk ahead', '', acc.1.v.09$target)) * 7
wave.t = acc.1.v.09[, list(wave = fn_getWave(week.t = week.t, loc.t = loc, vdates.dom = vdates.dom, variant = variant)), by = c('loc', 'variant', 'week.t')]
acc.1.v.09 = acc.1.v.09 %>% left_join(x = ., y = wave.t, by = c('loc', 'variant', 'week.t')) %>% data.table()

# acc.095.v.09 = res.eval %>% filter(fcast.deflat %in% c(.95,.9)) %>%
acc.095.v.09 = res.eval %>% filter(fcast.deflat != 1) %>%
  melt(., id.vars =  c('loc', 'variant', 'fcast.type', 'fcast.deflat', 'fcast.start.week','measure', 'metric', 'target', 'scenario', 'seasonality')) %>%
  dcast(., loc + variant + fcast.type + seasonality + measure + metric + fcast.start.week + target + scenario + variable ~ fcast.deflat) %>% 
  filter(variable == 'mean') %>%
  mutate(target2 = factor(target, levels = c(paste0(1:26, 'wk ahead'), 'peak intensity', 'peak week', 'total'),
...
acc.095.v.09$week.t = acc.095.v.09$fcast.start.week + as.numeric(gsub('wk ahead', '', acc.095.v.09$target)) * 7
wave.t = acc.095.v.09[, list(wave = fn_getWave(week.t = week.t, loc.t = loc, vdates.dom = vdates.dom, variant = variant)), by = c('loc', 'variant', 'week.t')]
acc.095.v.09 = acc.095.v.09 %>% left_join(x = ., y = wave.t, by = c('loc', 'variant', 'week.t')) %>% data.table()

acc.cp.deflat = rbind(acc.1.v.095, acc.1.v.09, acc.095.v.09)
rm(acc.1.v.095, acc.1.v.09, acc.095.v.09)

acc.cp.deflat = acc.cp.deflat %>% 
...
for(cp.t in cp.deflat_vec){
  for(sn.t in sn_vec){
    for(sce.t in sce_vec){
      ...
      rm(stats.t, ttda.t)
      ttda.t = da.t %>% filter(cp == cp.t & seasonality == sn.t & scenario == sce.t) %>% data.table()
...
stats.acc.cp.deflat = stats
save(stats.acc.cp.deflat, file = paste0(dir_res, 'stats.acc.cp.deflat_', stat.tag, '.RData'))
save(acc.cp.deflat, file = paste0(dir_res, 'acc.cp.deflat_', stat.tag, '.RData'))
rm(stats.acc.cp.deflat, acc.cp.deflat)
}
